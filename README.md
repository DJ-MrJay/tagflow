...
TagFlow is a desktop music tagger for local audio files. It scans what is already in your files, falls back to filename cleanup when tags are messy or missing, looks up likely matches from Apple/iTunes, and lets you review the result before anything is written.
...
- Searches Apple/iTunes for likely matches
...
- Creates a backup before overwriting desktop files
...
4. Enrich the chosen match with public Apple Music page data
5. Fill gaps from public lyrics and recording metadata sources when available
...
Before TagFlow overwrites a desktop file, it creates a backup:

- First backup: `song.ext.bak`
- Additional backups: `song.ext.bak.<timestamp>`

That is deliberate. The app is designed to avoid blind overwrites.
...
6. If needed, restore from the `.bak` file created beside the original.
...
- [electron/tag-service.ts](electron/tag-service.ts): desktop write path and backup handling
